...
class Pipe(Edge):
    ...
    def define_variables(self):
        ...
        pressure_t = daeVariableType("pressure_t", (Pa ** (1)), 0.1e+5, 50.0e+5, 1.0e+5, 1e-05)
    ...
    def eq_clean_wall(self):

        eq = self.CreateEquation("Biofilm", "Biofilm Formation - OFF")
        x = eq.DistributeOnDomain(self.x, eClosedClosed)
        eq.Residual = self.mf(x)

        print("+ edge clean wall {0}".format(self.x.NumberOfPoints))


    def eq_dirty_wall(self):
        ...
        print("+ edge dirty wall {0}".format(self.x.NumberOfPoints))
