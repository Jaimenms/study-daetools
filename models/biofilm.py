...
class Biofilm(daeModel):
    ...
    def DeclareEquations(self):
        ...
        x = eq.DistributeOnDomain(self.x, eOpenClosed)
        # eq.Residual = dt(A*self.rho()) + dk_dx(x) / self.L()
        eq.Residual = dk_dx(x)

        eq = self.CreateEquation("HeatBal", "Heat balance - T")
        x = eq.DistributeOnDomain(self.x, eOpenClosed)
        eq.Residual = self.rho() * self.cp() * dt(A * self.T(x)) + self.k(x) * self.cp() * d( self.T(x), self.x, eCFDM) / self.L() + self.Qout(x)

        eq = self.CreateEquation("TotalHeat", "Heat balance - Qout")
        x = eq.DistributeOnDomain(self.x, eClosedClosed)
        prandtl = self.cp() * self.mu() / self.kappa()
        #nusselt =  0.027 * (Re ** (4/5)) * (prandtl ** (1/3))
        nusselt = (self.fD(x) / 8.) * (Re - 1000.) * prandtl / ( 1. + 12.7 * Sqrt(self.fD(x) / 8.) * (prandtl ** 2 / 3) - 1.)
